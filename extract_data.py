    ...
    def import_data_for_model(self, window_size=10):
        ...
        for i in LLR_file_id_df['variant']:
            ...
            if (seq_center_at_variant_loc - W) < 1:
                start_index = 1
                end_index = W*2 + 1
            elif (seq_center_at_variant_loc + W) > len(LLR_df.columns):
                end_index = len(LLR_df.columns)
                start_index = len(LLR_df.columns) - W*2
            else:
                start_index = seq_center_at_variant_loc - W
                end_index = seq_center_at_variant_loc + W
            ...
            pred_2_struct_arr = np.array(list(predict_secondary_structure(seq_letters).values()))
            ...
            results.append((real_acid, variant_acid, position, esm1b_score, seq_arr, pred_2_struct_arr, variant_loc_LLR_arr, seq_LLR_arr, label))
        return results
...
model_data = np.empty(9)
for id in ClinVar_df_filtered['LLR_file_id'].unique():
    print(id)
    try:
        results = RetrieveData(id, ClinVar_df_filtered).import_data_for_model()
...
col_names = ['real_acid', 'variant_acid', 'position', 'esm1b_score', 'seq_arr', 'pred_2nd_struct_arr', 'variant_loc_LLR_arr', 'seq_LLR_arr', 'label']
extracted_data_df = pd.DataFrame(model_data[1:], columns=col_names)
extracted_data_df.to_pickle('extracted_data_W10.pkl')